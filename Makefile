...
VERSION_FILE := $(INCLUDE_DIR)/giorpc_version.h
get_version = $(shell grep -E '^ *\#define +GIORPC_VERSION_$1 +[0-9]+$$' $(VERSION_FILE) | sed 's/  */ /g' | cut -d ' ' -f 3)

VERSION_MAJOR := $(call get_version,MAJOR)
VERSION_MINOR := $(call get_version,MINOR)
VERSION_MICRO := $(call get_version,RELEASE)
...
LIB_SO := $(LIB_SONAME).$(VERSION_MINOR).$(VERSION_MICRO)
LIB := $(LIB_NAME).a
...
clean:
	...
	rm -fr debian/.debhelper debian/libgiorpc debian/libgiorpc-dev
	rm -f documentation.list debian/files debian/*.substvars
	rm -f debian/*.debhelper.log debian/*.debhelper debian/*~
	rm -f debian/libgiorpc.install debian/libgiorpc-dev.install
